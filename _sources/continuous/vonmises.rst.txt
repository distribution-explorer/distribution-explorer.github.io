...
It is perhaps easier to think of the Von Mises distribution as an approximation to the :ref:`normal` that lives on a periodic domain.
...
By convention, moments of distributions defined on periodic domains are usually calculated as the moments of the complex exponential of the angle :math:`y` instead of the angle itself. These are referred to as **circular moments**. The circular mean is
...
Perhaps confusingly, traditionally the "mean" of a distribution on a circular domain refers to the argument of the exponential, in this case :math:`\mu`.

Mean: :math:`\mu`

Circular variance: :math:`\displaystyle{1 - \frac{I_1(\kappa)}{I_0(\kappa)}}`
...
- In the limit of :math:`\kappa \to 0`, the Von Mises distribution becomes a :ref:`uniform` on the interval :math:`[-\pi, \pi)`.
...
- In Distributions.jl, the interval on which the distribution is defined is :math:`[\mu - \pi, \mu + \pi)`, like ``scipy.stats.vonmises_line``, with the PDF and CDF being evaluated similarly. By contrast, sampling from the Von Mises distribution using Distributions.jl returns values in the domain :math:`[\mu - \pi, \mu + \pi)`; there is no transformation of the location parameter.
